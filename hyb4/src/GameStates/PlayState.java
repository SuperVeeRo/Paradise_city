...
public class PlayState extends GameState {
	...
	private void populateDoors() {
		for (int i = 0; i<tileMap.getNumRows(); i++) {
			if (tileMap.getIndex(i,  0)==30) {
				Door door ;
				door = new Door(tileMap) ;
				door.setTilePosition(i, 0) ;
				doors.add(door) ;
			}
			if (tileMap.getIndex(i,  tileMap.getNumCols() - 1) == 30) {
				Door door ;
				door = new Door(tileMap) ;
				door.setTilePosition(i, tileMap.getNumCols() - 1) ;
				doors.add(door) ;
			}
		}
		for (int j = 0; j<tileMap.getNumCols(); j++) {
			if (tileMap.getIndex(0,  j)==30) {
				Door door ;
				door = new Door(tileMap) ;
				door.setTilePosition(0, j) ;
				doors.add(door) ;
			}
			if (tileMap.getIndex(tileMap.getNumRows()-1, j)== 30) {
				Door door ;
				door = new Door(tileMap) ;
				door.setTilePosition(tileMap.getNumRows()-1, j) ;
				doors.add(door) ;
			}
		}
	}
	...
	public void update() {
		...
		// check if a bed is in player's vicinity to suggest the "sleep" action
