...
public class TileMap {
	...
	public void loadTiles(String s) {
		...
		try {
			...
			numTileRows = tileset.getHeight() / tileSize;
			tiles = new Tile[numTileRows][numTileCols];
			...
			tileset = ImageIO.read(getClass().getResourceAsStream("/Tilesets/blonde-wood.png"));
			floor = tileset.getSubimage(5 * tileSize, 6 * tileSize, tileSize, tileSize);
	...
	public int getType(int row, int col) {
		int rc = map[row][col];
		int r = rc / numTileCols;
		int c = rc % numTileCols;
	...
	public void draw(Graphics2D g) {
		...
		for(int row = rowOffset; row < rowOffset + numRowsToDraw; row++) {
			...
			for(int col = colOffset; col < colOffset + numColsToDraw; col++) {
				...
				int r = rc / numTileCols;
				int c = rc % numTileCols;
				g.drawImage(floor, x + col * tileSize, y + row * tileSize, tileSize, tileSize, null );
				g.drawImage(
					tiles[r][c].getImage(),
					x + col * tileSize,
					y + row * tileSize,
